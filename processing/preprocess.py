...
#         'https://api.themoviedb.org/3/movie/{}?api_key=c0afbd4abf078ae3f317f7a4dda1e687'.format(movie_id))
#     data = response.json()
#     try:
...
def fetch_posters_and_trailer(movie_id):
    # Fetch movie details including the trailer
    response = requests.get(
        f'https://api.themoviedb.org/3/movie/{movie_id}?api_key=c0afbd4abf078ae3f317f7a4dda1e687&append_to_response=videos'
    )
    data = response.json()
    ...
    try:
        ...
        poster_url = "https://media.istockphoto.com/vectors/error-icon-vector-illustration-vector-id922024224?k=6&m=922024224&s=612x612&w=0&h=LXl8Ul7bria6auAXKIjlvb6hRHkAodTqyqBeA6K7R54="
...
def recommend(new_df, movie, pickle_file_path):
    ...
    #     rec_poster_list.append(fetch_posters_and_trailer(new_df.iloc[i[0]]['movie_id']))

    # return rec_movie_list, rec_poster_list

    rec_poster_list = []
    rec_trailer_list = []  # List to store trailer URLs

    for i in movie_list:
        rec_movie_list.append(new_df.iloc[i[0]]['title'])
        poster_url, trailer_url = fetch_posters_and_trailer(new_df.iloc[i[0]]['movie_id'])
        rec_poster_list.append(poster_url)
        rec_trailer_list.append(trailer_url)  # Store the trailer URL

    return rec_movie_list, rec_poster_list, rec_trailer_list  # Return the trailer list as well
...
def fetch_person_details(id_):
    data = requests.get(
        'https://api.themoviedb.org/3/person/{}?api_key=c0afbd4abf078ae3f317f7a4dda1e687'.format(id_)).json()
...
def get_details(selected_movie_name):
    ...
    this_poster,trailer_key = fetch_posters_and_trailer(movie_id)
    ...
    info = [this_poster, budget, genres, overview, release_date, revenue, runtime, available_lang, vote_rating,
            vote_count, movie_id, cast, director, lang, cast_id,trailer_key]
