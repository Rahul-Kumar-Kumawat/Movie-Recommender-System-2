...
from processing.preprocess import fetch_posters_and_trailer
displayed = []
...
def main():
    def initial_options():
        ...
        st.session_state.user_menu = streamlit_option_menu.option_menu(
            ...
            options=['Recommend me a similar movie', 'Describe me the movie', 'Check all Movies'],
        ...
        elif st.session_state.user_menu == 'Describe me the movie':
    ...
    # Unpack all three returned values
        movies, posters, trailers = preprocess.recommend(new_df, selected_movie_name, pickle_file_path)

        st.subheader(f'Best Recommendations {str}...')

        rec_movies = []
        rec_posters = []
        rec_trailers = []  # List to store trailer URLs
        cnt = 0
    # Adding only 5 unique recommendations
        for i, j in enumerate(movies):
            ...
            if j not in displayed:
                rec_movies.append(j)
                rec_posters.append(posters[i])
                rec_trailers.append(trailers[i])  # Store the trailer URL
                displayed.append(j)
                cnt += 1
    ...
    # Columns to display information of movies i.e. movie title and movie poster
        col1, col2, col3, col4, col5 = st.columns(5)
        with col1:
            st.image(rec_posters[0], caption=rec_movies[0])
            if rec_trailers[0]:
                st.markdown(f"[Watch Trailer]({rec_trailers[0]})")
        with col2:
            st.image(rec_posters[1], caption=rec_movies[1])
            if rec_trailers[1]:
                st.markdown(f"[Watch Trailer]({rec_trailers[1]})")
        with col3:
            st.image(rec_posters[2], caption=rec_movies[2])
            if rec_trailers[2]:
                st.markdown(f"[Watch Trailer]({rec_trailers[2]})")
        with col4:
            st.image(rec_posters[3], caption=rec_movies[3])
            if rec_trailers[3]:
                st.markdown(f"[Watch Trailer]({rec_trailers[3]})")
        with col5:
            st.image(rec_posters[4], caption=rec_movies[4])
            if rec_trailers[4]:
                st.markdown(f"[Watch Trailer]({rec_trailers[4]})")
    ...
    def display_movie_details():
        ...
        with st.container():
            ...
            with image_col:
                st.text('\n')
                # st.image(info[0])
                st.markdown(f'<a href="{info[-1]}" target="_blank"><img src="{info[0]}" alt="Movie Poster" style="width:300px;height:auto;"></a>', unsafe_allow_html=True)
        ...
        with st.container():
            ...
            for col in cols:
                if i < len(movies):
                    ...
                    poster_url, trailer_url = fetch_posters_and_trailer(movie_id)
                    
                
                    # Create a clickable link for the poster
                    with col:
                        if trailer_url:
                            st.markdown(f"[![{movies['title'][i]}]({poster_url})]( {trailer_url} )", unsafe_allow_html=True)
                        else:
                            st.image(poster_url, caption=movies['title'][i])
                    i += 1
